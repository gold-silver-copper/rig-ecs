//! ECS policy migration of the DeepSeek permission-control regression.
...
async fn run(client: deepseek::Client, streaming: bool) -> Result<()> {
    let file = PermissionFile::new("deepseek", if streaming { "stream" } else { "prompt" })?;
    let agent = client
