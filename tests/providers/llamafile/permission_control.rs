//! ECS policy migration of the live llamafile permission-control regression.
...
async fn run(streaming: bool) -> Result<()> {
    ...
    let file = PermissionFile::new("llamafile", if streaming { "stream" } else { "prompt" })?;
...
#[tokio::test]
#[ignore = "requires a local llamafile server"]
