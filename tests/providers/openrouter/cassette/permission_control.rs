//! ECS policy migration of the OpenRouter permission-control regression.
...
async fn run(client: rig::providers::openrouter::Client, streaming: bool) -> Result<()> {
    let file = PermissionFile::new("openrouter", if streaming { "stream" } else { "prompt" })?;
    let agent = client
        .agent(TOOL_MODEL)
        .preamble(PREAMBLE)
